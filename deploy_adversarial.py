...
def valid():
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("G_0") as scope:
            G_model = G_scale_LSTM(scope=scope,scale_index=0,height=20,width=18,length=20,batch_size = FLAGS.batch_size,layer_num_lstm=2,kernel_size=[5,5],kernel_num=[128,128],future_seq_length=20,flag_for_future =True,scope_string = "G_0")
	...
	saver = tf.train.Saver(tf.all_variables())
	    ...
	    G_feed_dict = {G_model.input_frames:dat[:,0:FLAGS.seq_start,:,:,:],G_model.future_frames:dat[:,FLAGS.seq_start:,:,:,:]}
            g_predict,g_reconstruct = sess.run([G_model.preds,G_model.decodes],feed_dict=G_feed_dict)
        #     """ training for G model """
    	  #  D_input_ = {D_model.input_frames:np.concatenate([dat[:,0:FLAGS.seq_start,:,:,:],g_predict],1)}
	    accuracy= []
	    for i in range(1,20):
	        D_input_ = {D_model.input_frames:np.concatenate((dat[:,i:20,:,:,:],g_predict[:,0:i,:,:,:]),axis=1)}
                d_predict = sess.run([D_model.preds],feed_dict= D_input_)[0]
	        accuracy.append(d_predict[:,0].mean())
       #     G_feed_dict = {G_model.input_frames:dat[:,0:FLAGS.seq_start,:,:,:],G_model.future_frames:dat[:,FLAGS.seq_start:,:,:,:],G_model.D_label:d_predict}
		...
		data_handler.DisplayData_Ecog(dat,rec=g_reconstruct,fut=g_predict,case_id=i,output_file =output_file)
	#
	    """ adding summary """
    	    #summary_writer.add_summary(g_summary_, step)
